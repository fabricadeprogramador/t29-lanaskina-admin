<template>
    <transacoes :movimentacao_empresa="todasEmpresas"></transacoes>
</template>

<script>
import transacoes from "../components/Transações";
import TransacoesHttp from "@/http/TransacoesHttp"
...
export default {
  ...
  methods:{
     async initialize(){
       this.todasEmpresas.splice(0, this.todasEmpresas.length); 

         let resposta = await TransacoesHttp.buscarTodos(); 
         
          if(resposta.status ===200){
            for (let i = 0; i < resposta.data.length; i++) {
            this.todasEmpresas.push(resposta.data[i]);
          }
